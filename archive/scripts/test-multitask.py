...
import gym
import os
from src.nature.conv_kernel import Transformer, KConvSet, get_kernel
...
dimensions = [
    ...
    skopt.space.Categorical(['sepconv1d', "k_conv1", "k_conv2", "k_conv3",
                             "deep", "wide_deep", "transformer"], name='last_layer'),
...
def check_feedback_shapes():
    feedbacks = [x for x in G.successors("source") if "feedback" in x]
    for feedback in feedbacks:
        source_id = feedback[:-9]
        if source_id not in G.nodes():
            G.remove_node(feedback)
            G.remove_node(f"{source_id}_out")
        else:
            node = G.node[feedback]
...
def get_IO_pairs():
    return zip(G.successors("source"), G.predecessors("sink"))
...
def trial(**kwargs):
    ...
    activation_distribution = [hp.p_tanh, hp.p_linear, hp.p_relu, hp.p_selu,
                               hp.p_elu, hp.p_sigmoid, hp.p_hard_sigmoid,
                               hp.p_exponential, hp.p_softmax, hp.p_softplus,
                               hp.p_softsign, hp.p_gaussian, hp.p_sin, hp.p_cos,
                               hp.p_swish]
    ...
    [print(k, v) for k, v in hp.items()]

    prepared = prepare_tasks(tasks)

    all_pairs = []
    for j in range(MODELS_PER_TRIAL):
        G = get_initial_graph(prepared, hp.initial_boxes)
        screenshot(G, i, j, 0)
        n_mutations = safe_sample(hp.min_mutations, hp.max_mutations)
        for k in range(n_mutations):
            mutate(hp)
            screenshot(G, i, j, k + 1)
        # differentiate(hp)
        # check_feedback_shapes()
        # screenshot(G, i, j, n_mutations + 1)
        # try:
        #     model, inputs, outputs = make_model(hp)
        # except Exception as e:
        #     print()
        #     print(e)
        #     print(e)
        ...
        #     print()
        #     continue
        # show_model(model, i, j, k)
        # current_pairs = get_IO_pairs()
        # log("GRAPH IO:")
        # [log(pair) for pair in current_pairs]
        # log("MODEL IO:")
        # [log(pair) for pair in zip(inputs, outputs)]
        # all_pairs.append((k, current_pairs))

    if i > TRIALS_PER_MOVIE:
        make_gif(IMAGE_PATH)
        i = 0
    i += 1

    [[log(i, pair) for pair in pairs] for i, pairs in all_pairs]
...
def main():
    ...
    try:
        ...
        log(e)
    os.mkdir(IMAGE_PATH)
    results = skopt.gp_minimize(trial, dimensions, verbose=True)
    print(results)
...
# def handle_recurrence(G):
#     screenshot(G, "cycles")
#     cycles = list(nx.simple_cycles(G))
#     print("cycles:", cycles)
#     for c, cycle in enumerate(cycles):
#         print(f"HANDLE RECURRENCE FOR {cycle}")
#
#         output_key = f"{cycle[0]}_out"
#         G.add_node(output_key, shape="triangle", color="red")
#         if output_key not in G.successors(cycle[0]):
#             G.add_path([cycle[0], output_key, "sink"])
#
#         input_key = f"{cycle[0]}_feedback"
#         G.add_node(input_key, shape="circle", color="blue")
#         if input_key not in G.successors("source"):
#             G.add_edge("source", input_key)
#
#         if len(cycle) == 1:
#             if cycle[0] not in G.successors(input_key):
#                 G.add_edge(input_key, cycle[0])
#             G.remove_edge(cycle[0], cycle[0])
#         else:
#             if cycle[-1] not in G.successors(input_key):
#                 G.add_edge(input_key, cycle[-1])
#             G.remove_edge(cycle[0], cycle[-1])
#     screenshot(G, "recurrent")
