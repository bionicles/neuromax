...
def get_image_encoder_output(agent, brick_id, input, output_shape):
    ...
    filters = [agent.pull_numbers(f"{brick_id}_{layer_number}_filters", MIN_FILTER_CONV, MAX_FILTER_CONV)
               for layer_number in num_conv_layers]
    conv_layer = preact_conv_2D(input, filters=filters[0], activation=conv_activation)
    for i in range(1, num_conv_layers):
        conv_layer = preact_conv_2D(conv_layer, filters=filters[i], activation=conv_activation)
    ...
    x_layer = L.Dense(x_units, activation=dense_activation)(flatten_layer)
...
def get_image_decoder_output(agent, brick_id, input, output_shape):
    ...
    filters = [agent.pull_numbers(f"{brick_id}_{layer_number}_filters", MIN_FILTER_DECONV, MAX_FILTER_DECONV)
