...
source: https://raw.githubusercontent.com/eminorhan/mixture-of-experts/master/DenseMoE.py
"""
import numpy as np
...
class DenseMoE(Layer):
    ...
    def build(self, input_shape):
        ...
        expert_init_lim = np.sqrt(
            3.0*self.expert_kernel_initializer_scale / (max(1., float(input_dim + self.units) / 2))
            )
        gating_init_lim = np.sqrt(
            3.0*self.gating_kernel_initializer_scale / (max(1., float(input_dim + 1) / 2))
            )

        self.expert_kernel = self.add_weight(
            shape=(input_dim, self.units, self.n_experts),
            initializer=RandomUniform(
                minval=-expert_init_lim,maxval=expert_init_lim),
            name='expert_kernel',
        ...
        self.gating_kernel = self.add_weight(
            ...
            initializer=RandomUniform(
                minval=-gating_init_lim, maxval=gating_init_lim),
            name='gating_kernel',
